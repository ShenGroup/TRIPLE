...
class Env:
    ...
    def __init__(
        ...
        # TODO: add function approxiamiation option and more adjustable feedback generation method. 
        
        # assert bandit_choice in ["Cluster", "Iterative", "FuncApprox", None], "Input a valid advanced method"
        assert bandit_choice in ["UCB", "SequentialHalving", "SuccessiveRejects", "TopK", "ContinuousRejects", "GSE", "Cluster", "Iterative", "FuncApprox"], "Input a valid advanced method"
        ...
        if load_from_file:
            ...
            if LLM_choice == "ChatGPT":
                self.candidate_prompts = asyncio.run(self.generate_candidate_prompt_async())
            else:
                self.candidate_prompts = self.generate_candidate_prompt()
            self.prompts_df = pd.DataFrame(self.candidate_prompts, columns = ["prompts"])
            self.__generate_embeddings()
            if bandit_choice == "Cluster":
                # Cluster enforce equal size/ budget proportional to the number of prompts
                self.__cluster_prompts()
        
        if bandit_choice not in ["Cluster", "Iterative", "FuncApprox"]:
        ...
        elif bandit_choice == "Iterative":
            self.phase1_budget = T // 2
            self.current_phase = 1
    ...
    def step(self):
        ...
        if self.t == self.phase1_budget and self.bandit_choice == "Cluster":
            ...
            self.prompt_bandit.num_pulls = num_pulls
