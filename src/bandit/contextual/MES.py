...
from botorch.acquisition.max_value_entropy_search import qMaxValueEntropy, MaxValueBase
...
class MES(BasicBandit):
    ...
    def __init__(self, num_arms: int = 0, T: int = 100, contexts: List[float] = []) -> None:
        ...
        self.candidate = np.zeros(self.num_arms)
    ...
    def update(self, 
        ...
        if len_batch % 16 == 0 or self.t == self.T:
            ...
            # Ensure that train_X and train_y are on the same device
            ...
            # Kernel setup
            matern_kernel = MaternKernel(nu=2.5, ard_num_dims=train_X.shape[-1], lengthscale_prior=GammaPrior(3.0, 6.0))
            matern_kernel_y = MaternKernel(nu=2.5, ard_num_dims=train_X.shape[-1], lengthscale_prior=GammaPrior(3.0, 6.0))

            # Covariance module setup
            covar_module = ScaleKernel(base_kernel=CombinedStringKernel(
                base_latent_kernel=matern_kernel,
                instruction_kernel=matern_kernel_y,
                latent_train=train_X.double(),
                instruction_train=train_y.double()))
            ...
            # bounds = torch.stack([torch.zeros(eval_X.shape[1]), torch.ones(eval_X.shape[1])])

            # candidate_set = torch.rand(eval_X.shape[0], bounds.size(1))
            # candidate_set = bounds[0] + (bounds[1] - bounds[0]) * candidate_set

            # Define the acquisition function
            # self.MES = qMaxValueEntropy(gp_model, candidate_set=candidate_set.to(device))
            # PI = LogExpectedImprovement(gp_model, train_y.max().item(), maximize=True)
            # MES = MaxValueBase(gp_model, num_mv_samples = 10)
            PI = LogExpectedImprovement(gp_model, train_y.max().item(), maximize=True)

            # Set bounds for optimization
            

            # Optimize the acquisition function to get the best candidate
            # breakpoint()
            try:
                # self.candidate = self.MES(eval_X.unsqueeze(1))
                self.candidate = PI(eval_X).detach().cpu().numpy()
